            ...
            mockDb.transaction = jest.fn().mockImplementation(async (cb: any) => {
                ...
                const trx: any = jest.fn().mockImplementation(() => {
                    ...
                    switch (callCount) {
                        ...
                        case 2:
                            return { where: jest.fn().mockReturnValue([createdUser]) };
                        case 3:
                            return { insert: jest.fn().mockResolvedValue([1]) };
                        case 4:
                            return { where: jest.fn().mockReturnValue([createdWallet]) };
                        default:
                            return { insert: jest.fn().mockResolvedValue([1]), where: jest.fn().mockReturnValue([]) };
                    }
                });
