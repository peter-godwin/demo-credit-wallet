...
import { KarmaCheckResult } from "./karma.types";
    ...
    async checkKarmaBlacklist(identity: string): Promise<KarmaCheckResult> {
        ...
        try {
            ...
            if (response.status === HttpStatus.OK && response.data?.data) {
                return {
                    isBlacklisted: true,
                    reason: response.data.data?.reason || "User is on the Karma blacklist",
                };
            }
...
export const checkKarmaBlacklist = (identity: string) => karmaService.checkKarmaBlacklist(identity);
